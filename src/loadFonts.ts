...
export async function loadFontPreviews(
	...
	// add customStyles
		// const customStyles = fontsArray
		// 	.filter(font => font.stylesheet)
		// 	.map(font => font.stylesheet)
		// 	.join('\n')
	...
	fontsArray.filter(font => font.stylesheet).forEach( fontItem => { fontStyles[fontItem.id] = String(fontItem.stylesheet) })
...
// Get Google Fonts stylesheet containing all requested styles, custom fonts from font.stylesheet)

		const fontStyle = font.stylesheet  || await getStylesheet([font], scripts, variants, false);
